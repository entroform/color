...
export class Color {
  ...
  set colorString(input: string) {
    ...
    } else if (COLOR_INPUT_REGEX.hex.test(input)) {
      this.hex = input;
    } else if (COLOR_INPUT_REGEX.rgb.test(input)) {
      this.rgbString = input;
    } else if (COLOR_INPUT_REGEX.rgba.test(input)) {
      this.rgbaString = input;
    } else if (COLOR_INPUT_REGEX.hsl.test(input)) {
      this.hslString = input;
    } else if (COLOR_INPUT_REGEX.hsla.test(input)) {
  ...
  set rgbString(input: string) {
    ...
    if (numbers) {
      const rgb = numbers.map(v => {
        return cycleNumber(parseFloat(v) / 255, 1);
      });
  ...
  set rgbaString(input: string) {
    ...
    if (numbers) {
      const rgba = numbers.map((value, index) => {
        if (index === 3) {
          return cycleNumber(parseFloat(value), 1);
        }

        return cycleNumber(parseFloat(value) / 255, 1);
      });
  ...
  set rgb255(rgb: ColorArray3) {
    this.r = cycleNumber(rgb[0] / 255, 1);
    this.g = cycleNumber(rgb[1] / 255, 1);
    this.b = cycleNumber(rgb[2] / 255, 1);
  }
  ...
  set rgb(rgb: ColorArray3) {
    this.r = cycleNumber(rgb[0], 1);
    this.g = cycleNumber(rgb[1], 1);
    this.b = cycleNumber(rgb[2], 1);
  }
  ...
  set rgba255(rgba: ColorArray4) {
    this.r = cycleNumber(rgba[0], 1);
    this.g = cycleNumber(rgba[1], 1);
    this.b = cycleNumber(rgba[2], 1);
    this.a = cycleNumber(rgba[3], 1);
  }
  ...
  set green255(green: number) {
    this.g = cycleNumber(green / 255, 1);
  }
  ...
  set hue(degrees: number) {
    ...
    hsl[0] = Math.abs(Math.round(cycleNumber(degrees, 359)));
  ...
  set saturation(saturation: number) {
    ...
    hsl[1] = cycleNumber(saturation, 1);
  ...
  public lerp(target: Color, t: number): this {
    this.r = lerp(t, this.r, target.r);
    this.g = lerp(t, this.g, target.g);
    this.b = lerp(t, this.b, target.b);
    this.a = lerp(t, this.a, target.a);
  ...
  static lerp(t: number, color_a: Color, color_b: Color): Color {
    ...
    color.r = lerp(t, color_a.r, color_b.r);
    color.g = lerp(t, color_a.g, color_b.g);
    color.b = lerp(t, color_a.b, color_b.b);
    color.a = lerp(t, color_a.a, color_b.a);
